...
function extractNumberAfterLabel(html, label, windowSize) {
  ...
  const slice = html.slice(
    idx + label.length,
    idx + label.length + (windowSize || 500)
  );
...
async function lookupNutriDatabaze(query) {
  ...
  const listUrl =
    "https://www.nutridatabaze.cz/vyhledavani-potravin/podle-abecedy/?letter=" +
    encodeURIComponent(letter);
  const listRes = await fetchText(listUrl, 9000);
  if (!listRes.ok)
    return {
      success: false,
      message: "NutriDatabázi se teď nepodařilo načíst.",
    };
  ...
  if (!anchors.length)
    return { success: false, message: "V NutriDatabázi se nic nenašlo." };
  ...
  if (!best || bestScore < 0)
    return {
      success: false,
      message: "V NutriDatabázi jsem nenašel odpovídající potravinu.",
    };

  const detailUrl = "https://www.nutridatabaze.cz/potraviny/?id=" + best.id;
  const detailRes = await fetchText(detailUrl, 9000);
  if (!detailRes.ok)
    return {
      success: false,
      message: "Nepodařilo se načíst detail potraviny z NutriDatabáze.",
    };
  ...
  if (kcal == null)
    return {
      success: false,
      message: "Potravina nalezena, ale hodnoty se nepodařilo přečíst.",
    };
...
function extractFromKtHtml(html) {
  ...
  const carbsM = /data\.foodstuff\.carbohydrate\}\}\s*([\d.,]+)\s*g/i.exec(
    html
  );
...
async function findKtUrlViaDuckDuckGo(query) {
  ...
  let href = m ? m[1] : null;
  if (!href) {
    // fallback: jakýkoli odkaz na kaloricketabulky.cz/potraviny/...
    const generic =
      /href="([^"]*kaloricketabulky\.cz%2Fpotraviny%2F[^"&]*|[^"]*kaloricketabulky\.cz\/potraviny\/[^"?&]*)"/i.exec(
        res.html
      );
  ...
  if (!/kaloricketabulky\.cz\/potraviny\//.test(finalUrl)) return null;
  return finalUrl;
}
...
async function lookupKalorickeTabulky(query) {
  ...
  if (slug) {
    const directRes = await fetchText(
      "https://www.kaloricketabulky.cz/potraviny/" + slug,
      9000
    );
    if (directRes.ok) {
      ...
      if (data)
        return Object.assign(
          { success: true, source: "KalorickéTabulky.cz", per: "100 g" },
          data
        );
    }
  }
  ...
  const foundUrl = await findKtUrlViaDuckDuckGo(q);
  if (foundUrl) {
    const res2 = await fetchText(foundUrl, 9000);
    if (res2.ok) {
  ...
  return {
    success: false,
    message:
      "Na Kalorických tabulkách se tuto položku nepodařilo najít. Zkus upravit název (např. bez značky) nebo zadej hodnoty ručně.",
  };
}
...
export default async function handler(req, res) {
  ...
  if (!q.trim()) {
    res
      .status(400)
      .json({ success: false, message: "Chybí parametr q (název potraviny)." });
    return;
  }
  ...
  try {
    ...
    res
      ...
      .json({
        success: false,
        message:
          "Nastala chyba při vyhledávání: " + String((e && e.message) || e),
      });
  }
}
